...
namespace IntegrationTests.CalculateUtility
{
    ...
    public class IT_ConsiderationUsAverageAiContextDecisionBucketUcsHighest
    {
        ...
        private List<Consideration> CreateConsiderations(int numberOfConsiderations, float min = 0f, float max = 1f,
        ...
        private List<Decision> CreateDecisionsWithConsiderations(int numberOfDecisions, int numberOfConsiderations,
            float min = 0f, float max = 1f, float returnValue = -1f)
        {
        ...
        private List<Bucket> CreateBucketsWithDecisions(int numberOfBuckets, int numberOfDecisions,
            int numberOfConsiderations, float min = 0f, float max = 1f, float returnValue = -1f)
        {
            ...
            for (var i = 0; i < numberOfBuckets; i++)
            {
                var b = new Bucket();
                result.Add(b);
                var decisions = CreateDecisionsWithConsiderations(numberOfDecisions, numberOfConsiderations, min, max,
                    returnValue);
